...
def main(args):
    ...
    logfile = open("log/log_msm2_%s.txt"%start_time, "w")
    ...
    load_model = False ########
    MODEL_PATH = "log/msm2_%s.pt"%start_time
    model = models.msm2_xxt(num_classes = TinyImageNet.N_CLASS) ########
    if load_model:
    ...
    pd.DataFrame(log).to_csv("log/log_msm2_%s.csv"%start_time, index = False, header = True)
    logfile.close()
