...
class UnfoldLayer2(nn.Module):
    def __init__(
        ...
        self.interaction_channels = up_channels ##############################
        ...
        # self.shortcut_project = nn.Linear(self.interaction_channels, down_channels) ############### 
        self.index_rect = nn.Linear(self.interaction_channels, num_rects * 4)
        self.project_rects = nn.Sequential(
            nn.Linear(in_channels, down_channels),
            nn.GELU(),
        )
        self.mask = nn.Linear(self.interaction_channels, num_rects * self.groups)
    ...
    def forward(self, x, I_tuple):
        ...
        # shortcut = self.shortcut_project(y) ###################
        # y = self.norm0(y) ##### 
...
class FuseLayer2(nn.Module):
    def __init__(self, down_channels, up_channels, out_channels, bunch_relations, up_bunches, groups):
        ...
        ##################
        self.query = nn.Parameter(torch.zeros(1, sum(self.up_bunches), self.groups, self.c_q // self.groups))
    ...
    def forward(self, u, x):
        ######### 修改：不以x_{i-1}生成query，而是直接使用可学习参数作为query
        # 接受输入：u_{i}, x_{i - 1}，
        # 输出： u_{i - 1}
        ...
        qs = torch.split(self.query, self.up_bunches, dim = 1)
        ...
        for i, secs in enumerate(zip(*u_)):
            ...
            ######### 还要加入分组（多头）注意力
            q = qs[i].expand(B, x_i.shape[1], self.groups, self.c_q // self.groups)
