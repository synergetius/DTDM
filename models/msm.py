...
class UnfoldLayer(nn.Module):
    def __init__(
        ...
        for to_number in self.to_numbers:
            self.interactions.append(nn.Linear(up_channels, up_channels * to_number))
        self.index_rect = nn.Linear(up_channels, num_rects * 4)
        self.project_rects = nn.Sequential(
            nn.Linear(in_channels, down_channels),
            nn.GELU(),
        )
        self.mask = nn.Linear(up_channels, num_rects * self.groups)
    ...
    def forward(self, x, I_tuple):
        ...
        for i, (bunch, interaction, to_number) in enumerate(zip(bunches, self.interactions, self.to_numbers)):
            _, n, __ = bunch.shape
            y_ = interaction(bunch) # (B, n_i, to_number * C)
            # print('!', y_.shape)
            # print('!!', (B, n * to_number, C))
            y_ = y_.view(B, n * to_number, C)
        ...
        M = y.shape[1]
        # index_rect
        p = self.index_rect(y).view(B, y.shape[1], self.num_rects, 4) # (B, M, num_rects * 4)
        rects = self.sample_rects(p, I_tuple) # (B, M, num_rects, in_channels)
        rects = self.project_rects(rects)
        mask = self.mask(y) # (B, M, num_rects * groups)
...
class FuseLayer(nn.Module):
    def __init__(self, down_channels, up_channels, out_channels, bunch_relations, up_bunches, groups):
        ...
        self.up_bunches = tuple(up_bunches)
        self.project_key = nn.Linear(down_channels, up_channels) # 从u_{i}投影得到key
        self.project_query = nn.Linear(up_channels, up_channels) # 暂时以top_channels作为q,k统一的维度
    ...
    def forward(self, u, x):
        ...
        for i, secs in enumerate(zip(*u_)):
            ...
            k = self.project_key(u_i).view(B, u_i.shape[1], u_i.shape[2], self.groups, C // self.groups) 
            #k: (B, n_i, c_{i1} + ... + c_{il}, G, C / G)
            ######### 还要加入分组（多头）注意力
            q = self.project_query(x_i).view(B, x_i.shape[1], self.groups, C // self.groups) # (B, n_i, G, C / G)
            attn = torch.einsum('b n u g c, b n g c -> b n u g', k, q) / (C ** 0.5) # (B, n_i, c_{i1} + ... + c_{il}, G)
