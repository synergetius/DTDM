...
def main(args):
    ...
    logfile = open("log/log_%s_%s.txt"%(model_name, start_time), "w")
    ...
    load_model = False ########
    MODEL_PATH = "log/%s_%s.pt"%(model_name, start_time)
    ...
    pd.DataFrame(log).to_csv("log/log_%s_%s.csv"%(model_name, start_time), index = False, header = True)
    logfile.close()
